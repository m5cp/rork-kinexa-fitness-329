...
struct ActiveSessionView: View {
    ...
    private var exerciseCard: some View {
        ...
        return AnyView(VStack(spacing: 0) {
            ...
            .shadow(color: KinexaTheme.accent.opacity(0.2), radius: 24, y: 16)
        })
    }
    ...
    private var timerSection: some View {
        VStack(spacing: 14) {
            ZStack {
                Circle()
                    .stroke(KinexaTheme.cardSoft, lineWidth: 6)
            ...
            HStack(spacing: 12) {
                Button {
                    ...
                    .frame(maxWidth: .infinity)
                    .frame(height: 44)
                    .background(workoutTimer.isRunning ? KinexaTheme.warning : KinexaTheme.accent)
                    .clipShape(RoundedRectangle(cornerRadius: 12))
                }
                .buttonStyle(PressScaleButtonStyle())
    ...
    private var timerTextColor: Color {
        if workoutTimer.timeRemaining <= 10 && workoutTimer.timeRemaining > 0 && workoutTimer.isRunning {
            return KinexaTheme.warning
        }
        if workoutTimer.timeRemaining <= 0 {
            return KinexaTheme.success
        }
        return .white
    }
                ...
                HStack(spacing: 12) {
                    ForEach([30, 60, 90], id: \.self) { seconds in
                        Button {
                            ...
                            Text("\(seconds)s")
                                .font(.subheadline.weight(.semibold))
                                .foregroundStyle(restDuration == seconds ? .white : KinexaTheme.secondaryText)
                                .frame(maxWidth: .infinity)
                                .frame(height: 40)
                                .background(restDuration == seconds ? KinexaTheme.accent.opacity(0.3) : KinexaTheme.cardSoft)
                ...
                Button {
                    ...
                    .overlay {
                        RoundedRectangle(cornerRadius: 14)
                            .stroke(KinexaTheme.accent.opacity(0.25))
                    }
            ...
            ForEach(Array(exercises.enumerated()), id: \.element.id) { index, exercise in
                Button {
                    ...
                    HStack(spacing: 12) {
                        ZStack {
                            Circle()
                                .fill(exercise.isCompleted ? KinexaTheme.success : (index == currentIndex ? KinexaTheme.accent : KinexaTheme.cardSoft))
                            ...
                            if exercise.isCompleted {
                                ...
                                Text("\(index + 1)")
                                    .font(.caption2.weight(.bold))
                                    .foregroundStyle(index == currentIndex ? .white : KinexaTheme.tertiaryText)
                            }
                        }
                    ...
                    .padding(12)
                    .background(index == currentIndex ? KinexaTheme.accent.opacity(0.08) : .clear)
                    .clipShape(RoundedRectangle(cornerRadius: 14))
                    .overlay {
                        if index == currentIndex {
                            RoundedRectangle(cornerRadius: 14)
