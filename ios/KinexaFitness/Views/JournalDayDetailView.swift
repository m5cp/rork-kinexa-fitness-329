...
struct JournalDayDetailView: View {
    ...
    private var overviewGrid: some View {
        LazyVGrid(columns: [GridItem(.flexible(), spacing: 12), GridItem(.flexible(), spacing: 12)], spacing: 12) {
            overviewTile(icon: "figure.mixed.cardio", value: "\(totalActivityCount)", label: "Workouts", color: KinexaTheme.success, gradient: [KinexaTheme.success, KinexaTheme.success.opacity(0.6)])
            ...
            overviewTile(icon: "shoeprints.fill", value: "\(steps)", label: "Steps", color: KinexaTheme.accent, gradient: [KinexaTheme.accent, KinexaTheme.accent2])
                    ...
                    ForEach(record.exercises.prefix(6)) { exercise in
                        HStack(spacing: 8) {
                            Image(systemName: exercise.isCompleted ? "checkmark.circle.fill" : "circle")
                                .font(.system(size: 10))
                                .foregroundStyle(exercise.isCompleted ? KinexaTheme.success : KinexaTheme.tertiaryText.opacity(0.5))
                            Text(exercise.name)
