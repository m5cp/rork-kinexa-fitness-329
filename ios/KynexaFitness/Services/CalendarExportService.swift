...
final class CalendarExportService {
    ...
    func resyncWeeklyPlanFromDate(_ plan: WeeklyPlan, from startDate: Date) async -> ExportResult {
        ...
        for day in affectedDays {
            removeKynexaEvents(on: day.date, prefix: "PT:")
        }
    ...
    func resyncWODPlanFromDate(_ plan: WODPlan, from startDate: Date) async -> ExportResult {
        ...
        for day in affectedDays {
            removeKynexaEvents(on: day.date, prefix: "WOD:")
        }
    ...
    func resyncSingleDay(on date: Date, title: String, prefix: String, notes: String, durationMinutes: Int) async -> ExportResult {
        ...
        removeKynexaEvents(on: date, prefix: prefix)
    ...
    func removeAllKynexaEventsForPlan(dates: [Date], prefix: String) {
    ...
    func removeAllKynexaEventsOnDate(_ date: Date) {
        guard EKEventStore.authorizationStatus(for: .event) == .fullAccess else { return }
        removeKynexaEvents(on: date, prefix: "PT:")
        removeKynexaEvents(on: date, prefix: "WOD:")
        removeKynexaEvents(on: date, prefix: "Unit PT:")
    }

    private func removeKynexaEvents(on date: Date, prefix: String) {
        let cal = Calendar.current
