...
struct MyPTPlanSheet: View {
        ...
        VStack(spacing: 12) {
            if !isPlanApproved {
                ...
                VStack(spacing: 8) {
                    ...
                    .frame(maxWidth: .infinity)
                    .frame(height: 48)
                    .background(KynexaTheme.success.opacity(0.1))
                    ...
                    Button {
                        ...
                        HStack(spacing: 8) {
                            if calendarService.isExporting {
                                ProgressView().tint(KynexaTheme.accent)
                            } else {
                ...
                HStack(spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text(day.isRestDay ? "Recovery & Mobility" : day.title)
                            .font(.subheadline.weight(.semibold))
                            .foregroundStyle(day.isRestDay ? KynexaTheme.secondaryText : KynexaTheme.primaryText)
                            .lineLimit(1)
                        if !day.isRestDay {
                ...
                HStack(spacing: 8) {
                    ForEach([2, 4, 6, 8, 12], id: \.self) { weeks in
                        Button {
                            ...
                            .foregroundStyle(selectedWeeks == weeks ? .white : KynexaTheme.secondaryText)
                            .frame(maxWidth: .infinity)
                            .frame(height: 56)
                            .background(selectedWeeks == weeks ? KynexaTheme.accent : Color.white.opacity(0.06))
                            .clipShape(RoundedRectangle(cornerRadius: 12))
                            .overlay {
        ...
        return Button {
            ...
            HStack(spacing: 14) {
                Image(systemName: goal.icon)
                    .font(.body.weight(.semibold))
                    .foregroundStyle(isSelected ? .white : KynexaTheme.accent)
                    .frame(width: 38, height: 38)
                    .background(isSelected ? KynexaTheme.accent : KynexaTheme.accent.opacity(0.12))
            ...
            .padding(12)
            .background(isSelected ? KynexaTheme.accent.opacity(0.08) : Color.white.opacity(0.03))
            .clipShape(RoundedRectangle(cornerRadius: 14))
            .overlay {
                RoundedRectangle(cornerRadius: 14)
                    .stroke(isSelected ? KynexaTheme.accent.opacity(0.3) : KynexaTheme.border)
            }
        }
        .buttonStyle(.plain)
    }
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            LazyVGrid(columns: [
                ...
                ForEach(uniqueFocuses, id: \.rawValue) { focus in
                    ...
                    HStack(spacing: 6) {
                        Circle()
                            .fill(KynexaTheme.accent.opacity(0.6))
    ...
    private func planHeaderBadge(_ plan: WeeklyPlan) -> some View {
        HStack(spacing: 12) {
            Image(systemName: selectedGoal.icon)
                ...
                .foregroundStyle(.white)
                .frame(width: 44, height: 44)
                .background(KynexaTheme.accent.opacity(0.3))
                .clipShape(RoundedRectangle(cornerRadius: 12))
            ...
            ZStack {
                ...
                Circle()
                    ...
                    .stroke(KynexaTheme.accent, style: StrokeStyle(lineWidth: 3, lineCap: .round))
                    .frame(width: 40, height: 40)
    ...
    private func weekOverviewHeader(_ plan: WeeklyPlan) -> some View {
        ...
        .shadow(color: KynexaTheme.accent.opacity(0.2), radius: 20, y: 12)
        .opacity(animateCards ? 1 : 0)
        .offset(y: animateCards ? 0 : 10)
    }
    ...
    private func dayRow(_ day: WorkoutDay, offset: Int) -> some View {
        ...
        HStack(spacing: 14) {
            VStack(spacing: 2) {
                ...
                Text(dayNumber(day.date))
                    ...
                    .foregroundStyle(
                        day.isCompleted ? KynexaTheme.success :
                        isToday ? KynexaTheme.accent :
                        KynexaTheme.secondaryText
                    )
            }
            .frame(width: 36)
            ...
            if day.isRestDay {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    Text(day.title)
                        .font(.subheadline.weight(.semibold))
                        .foregroundStyle(day.isCompleted ? KynexaTheme.secondaryText : KynexaTheme.primaryText)
                        .lineLimit(1)
        ...
        .padding(.vertical, 12)
        .background(
            isToday ? KynexaTheme.accent.opacity(0.08) :
            KynexaTheme.card.opacity(day.isCompleted ? 0.5 : 1)
        )
        .overlay {
            RoundedRectangle(cornerRadius: 14)
                .stroke(
                    isToday ? KynexaTheme.accent.opacity(0.2) :
                    day.isCompleted ? KynexaTheme.success.opacity(0.1) :
                    KynexaTheme.border
                )
        }
        .clipShape(RoundedRectangle(cornerRadius: 14))
                ...
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 24) {
                        VStack(spacing: 16) {
                            ...
                            Text("Scan with Kynexa Fitness to import this plan")
                                .font(.caption)
                                .foregroundStyle(KynexaTheme.tertiaryText)
                                .multilineTextAlignment(.center)
                        }
