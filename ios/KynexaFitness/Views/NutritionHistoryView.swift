...
struct NutritionHistoryView: View {
    ...
    private var weeklyCalorieChart: some View {
        ...
        return VStack(alignment: .leading, spacing: 16) {
                ...
                ForEach(Array(data.enumerated()), id: \.offset) { _, item in
                    VStack(spacing: 6) {
                        ...
                        RoundedRectangle(cornerRadius: 6)
                            .fill(
                                LinearGradient(
                                    colors: isOverGoal ? [KynexaTheme.warning, KynexaTheme.warning.opacity(0.6)] : [KynexaTheme.success, KynexaTheme.success.opacity(0.6)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
                        ...
                        Text(dayAbbrev(item.date))
                            ...
                            .foregroundStyle(Calendar.current.isDateInToday(item.date) ? KynexaTheme.success : KynexaTheme.tertiaryText)
                    }
            ...
            HStack(spacing: 8) {
                Circle().fill(KynexaTheme.success).frame(width: 6, height: 6)
                Text("Under goal")
                    .font(.system(size: 9, weight: .medium))
                    .foregroundStyle(KynexaTheme.tertiaryText)
                Circle().fill(KynexaTheme.warning).frame(width: 6, height: 6)
                Text("Over goal")
                    .font(.system(size: 9, weight: .medium))
                    .foregroundStyle(KynexaTheme.tertiaryText)
                Spacer()
                Rectangle()
                    .fill(KynexaTheme.tertiaryText.opacity(0.4))
            ...
            ForEach(days, id: \.timeIntervalSince1970) { date in
                ...
                HStack(spacing: 12) {
                    VStack(spacing: 2) {
                        Text(dayAbbrev(date))
                            .font(.system(size: 10, weight: .bold))
                            .foregroundStyle(isToday ? KynexaTheme.success : KynexaTheme.tertiaryText)
                        Text("\(calendar.component(.day, from: date))")
                            .font(.subheadline.weight(isToday ? .bold : .semibold))
                            .foregroundStyle(isToday ? KynexaTheme.primaryText : KynexaTheme.secondaryText)
                    }
                    .frame(width: 36)
                    ...
                    CircularProgressView(progress: progress, size: 28, lineWidth: 3, color: n.calories > nutritionVM.dailyGoal.calories ? KynexaTheme.warning : KynexaTheme.success)
                }
                .padding(12)
                .background(isToday ? KynexaTheme.accent.opacity(0.06) : KynexaTheme.card)
                .clipShape(.rect(cornerRadius: 14))
                .overlay {
                    RoundedRectangle(cornerRadius: 14)
                        .stroke(isToday ? KynexaTheme.accent.opacity(0.2) : KynexaTheme.border)
                }
            }
        }
    }
