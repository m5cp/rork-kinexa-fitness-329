...
struct TrainingCalendarView: View {
    ...
    private var calendarSyncCard: some View {
        HStack(spacing: 14) {
            Image(systemName: vm.isCalendarSyncEnabled ? "calendar.badge.checkmark" : "calendar")
                ...
                .foregroundStyle(vm.isCalendarSyncEnabled ? KynexaTheme.success : KynexaTheme.accent)
                .frame(width: 44, height: 44)
                .background((vm.isCalendarSyncEnabled ? KynexaTheme.success : KynexaTheme.accent).opacity(0.12))
        ...
        return Button {
            ...
            VStack(spacing: 4) {
                Text("\(calendar.component(.day, from: date))")
                    ...
                    .foregroundStyle(
                        isSelected ? .white :
                        !isCurrentMonth ? KynexaTheme.tertiaryText.opacity(0.4) :
            ...
            .background {
                if isSelected {
                    RoundedRectangle(cornerRadius: 10)
                        ...
                        .shadow(color: KynexaTheme.accent.opacity(0.3), radius: 6, y: 3)
                } else if isToday {
                    RoundedRectangle(cornerRadius: 10)
                        .stroke(KynexaTheme.accent.opacity(0.4), lineWidth: 1)
                }
            }
        }
    }
    ...
    private func statusIndicator(_ status: AppViewModel.CalendarWorkoutStatus?, isSelected: Bool) -> some View {
        ...
        case .completed:
            Image(systemName: "checkmark")
                ...
                .foregroundStyle(isSelected ? .white : KynexaTheme.success)
        case .planned:
            Circle()
                .fill(isSelected ? .white.opacity(0.7) : KynexaTheme.accent)
                .frame(width: 5, height: 5)
        case .missed:
            Circle()
                .fill(isSelected ? .white.opacity(0.4) : KynexaTheme.tertiaryText.opacity(0.4))
                .frame(width: 5, height: 5)
        case nil:
    ...
    private func workoutEntryCard(_ entry: AppViewModel.CalendarWorkoutEntry) -> some View {
        HStack(spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                Text(entry.title)
                    .font(.subheadline.weight(.bold))
                    .foregroundStyle(entry.status == .missed ? KynexaTheme.tertiaryText : KynexaTheme.primaryText)
                    .lineLimit(1)
    ...
    private var emptyDateState: some View {
        VStack(spacing: 14) {
            Image(systemName: "calendar.badge.exclamationmark")
                .font(.system(size: 32))
                .foregroundStyle(KynexaTheme.accent.opacity(0.3))
    ...
    private func statusColor(_ status: AppViewModel.CalendarWorkoutStatus) -> Color {
        ...
        case .planned: return KynexaTheme.accent
        case .missed: return KynexaTheme.tertiaryText
        }
    }

    private func sourceColor(_ source: WorkoutSource) -> Color {
        switch source {
        case .individual: return KynexaTheme.accent
