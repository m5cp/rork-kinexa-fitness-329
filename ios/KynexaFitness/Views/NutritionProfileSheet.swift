...
struct NutritionProfileSheet: View {
    ...
    private var stepIndicator: some View {
        HStack(spacing: 6) {
            ForEach(0..<totalSteps, id: \.self) { step in
                Capsule()
                    .fill(step <= currentStep ? KynexaTheme.accent : KynexaTheme.cardSoft)
                    .frame(height: 4)
            }
        }
    }
                ...
                HStack(spacing: 12) {
                    ForEach(BiologicalSex.allCases, id: \.rawValue) { option in
                        ...
                        Button {
                            ...
                            .padding(.vertical, 16)
                            .background(isSelected ? KynexaTheme.accent : KynexaTheme.card)
                            .clipShape(.rect(cornerRadius: 14))
                            .overlay {
                                if !isSelected {
                                    RoundedRectangle(cornerRadius: 14)
    ...
    private func activityLevelRow(_ level: ActivityLevel, selection: Binding<ActivityLevel>) -> some View {
        ...
        return Button {
            ...
            .padding(14)
            .background(isSelected ? KynexaTheme.accent.opacity(0.08) : KynexaTheme.card)
                    ...
                    Button {
                        ...
                        VStack(spacing: 10) {
                            ...
                            Text(goal.subtitle)
                                ...
                                .foregroundStyle(isSelected ? .white.opacity(0.8) : KynexaTheme.tertiaryText)
                        ...
                        .background(isSelected ? Color(hex: goal.color) : KynexaTheme.card)
                        .clipShape(.rect(cornerRadius: 16))
                        .overlay {
                            if !isSelected {
                                RoundedRectangle(cornerRadius: 16)
        ...
        .background {
            ZStack {
                ...
                RoundedRectangle(cornerRadius: 20)
                    .fill(
                        LinearGradient(
                            colors: [KynexaTheme.success.opacity(0.05), .clear],
                            startPoint: .topLeading,
                            endPoint: .bottomTrailing
                        )
                    )
                RoundedRectangle(cornerRadius: 20)
                    .stroke(KynexaTheme.success.opacity(0.15))
            }
        }
    ...
    private var navigationButtons: some View {
        HStack(spacing: 12) {
            ...
            Button {
                ...
                .background(
                    LinearGradient(
                        colors: [KynexaTheme.accent, KynexaTheme.brandGreenLight],
                        startPoint: .leading,
                        endPoint: .trailing
                    )
                )
        ...
        .padding(.vertical, 12)
        .background {
            KynexaTheme.background
                .overlay(alignment: .top) {
                    Rectangle().fill(KynexaTheme.border).frame(height: 0.5)
                }
