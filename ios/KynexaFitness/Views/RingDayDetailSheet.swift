...
struct RingDayDetailSheet: View {
    ...
    private var ringsHeroCard: some View {
        ...
        .elevatedCardShadow()
        .overlay { RoundedRectangle(cornerRadius: 20).stroke(KynexaTheme.border) }
    }
    ...
    private func ringPill(_ ring: RingType) -> some View {
        ...
        return VStack(spacing: 4) {
            ...
            Text("\(Int(prog * 100))%")
                ...
                .foregroundStyle(closed ? ring.color : KynexaTheme.tertiaryText)
        }
        .frame(maxWidth: .infinity)
        .padding(.vertical, 8)
        ...
        sectionCard(title: "Fitness", icon: "figure.run", color: RingType.fitness.color) {
            VStack(spacing: 10) {
                ForEach(dayWorkouts) { w in
                    activityRow(icon: "dumbbell.fill", title: w.title, subtitle: "\(w.exerciseCount) exercise\(w.exerciseCount == 1 ? "" : "s")", color: KynexaTheme.accent)
                }
    ...
    private var mealsSection: some View {
        sectionCard(title: "Meals", icon: "fork.knife", color: RingType.meals.color) {
            VStack(spacing: 10) {
                HStack(spacing: 12) {
                    macroPill(label: "Cal", value: "\(dayNutrition.calories)", color: KynexaTheme.accent)
                ...
                HStack(spacing: 14) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        Text(closed ? "Goal met · \(ringsVM.goals.sleepHours)+ hrs" : "\(ringsVM.goals.sleepHours - sleep.hours) hrs short of goal")
                            .font(.caption.weight(.medium))
                            .foregroundStyle(closed ? KynexaTheme.success : KynexaTheme.secondaryText)
                    }
                    Spacer(minLength: 0)
                }
            } else {
    ...
    private var pointsCard: some View {
        ...
        .elevatedCardShadow()
        .overlay { RoundedRectangle(cornerRadius: 16).stroke(KynexaTheme.border) }
    }
