        ...
        return Button {
            ...
            VStack(spacing: 4) {
                ...
                Text("\(calendar.component(.day, from: date))")
                    ...
                    .foregroundStyle(isFuture ? KynexaTheme.tertiaryText.opacity(0.5) :
                                    (isToday ? KynexaTheme.accent : KynexaTheme.secondaryText))
            }
            .frame(maxWidth: .infinity)
            .padding(.vertical, 4)
            .background(
                RoundedRectangle(cornerRadius: 10)
                    .fill(isToday ? KynexaTheme.accent.opacity(0.08) : .clear)
            )
        }
        .buttonStyle(.plain)
        .disabled(isFuture)
