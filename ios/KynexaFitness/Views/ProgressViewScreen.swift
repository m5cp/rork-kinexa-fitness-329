...
struct ProgressViewScreen: View {
    ...
    private var topSummaryHeader: some View {
        HStack(alignment: .firstTextBaseline) {
            ...
            if vm.streak > 0 {
                ...
                .padding(.vertical, 7)
                .background(KynexaTheme.warning.opacity(0.1))
                .clipShape(Capsule())
                .overlay {
                    Capsule().stroke(KynexaTheme.warning.opacity(0.2), lineWidth: 0.5)
                }
            }
        }
        .opacity(appeared ? 1 : 0)
        ...
        .background {
            ZStack {
                ...
                RoundedRectangle(cornerRadius: 24)
                    .fill(
                        LinearGradient(
                            colors: [KynexaTheme.accent.opacity(0.06), .clear],
                            startPoint: .topLeading,
                            endPoint: .bottomTrailing
                        )
                    )
                RoundedRectangle(cornerRadius: 24)
                    .stroke(KynexaTheme.accent.opacity(0.15))
            }
        }
        .clipShape(RoundedRectangle(cornerRadius: 24))
        .shadow(color: KynexaTheme.accent.opacity(0.06), radius: 20, y: 10)
        .opacity(appeared ? 1 : 0)
        .offset(y: appeared ? 0 : 12)
    }
                ...
                HStack(spacing: 5) {
                    ForEach(weekDates, id: \.self) { date in
                        ...
                        VStack(spacing: 6) {
                            Text(dayAbbrev(date))
                                .font(.caption2.weight(.bold))
                                .foregroundStyle(isToday ? KynexaTheme.accent : KynexaTheme.tertiaryText)
    ...
    private func weekDotFill(status: AppViewModel.CalendarWorkoutStatus?, isToday: Bool) -> Color {
        switch status {
        case .completed: return KynexaTheme.success
        case .planned: return isToday ? KynexaTheme.accent.opacity(0.25) : KynexaTheme.accent.opacity(0.15)
        case .missed: return KynexaTheme.tertiaryText.opacity(0.15)
        case nil: return isToday ? KynexaTheme.accent.opacity(0.12) : KynexaTheme.cardSoft
        }
    }
            ...
            if weekData.allSatisfy({ $0.count == 0 }) {
                ...
                Chart {
                    ForEach(weekData, id: \.label) { week in
                        ...
                        .foregroundStyle(
                            LinearGradient(
                                colors: [KynexaTheme.accent, KynexaTheme.accent.opacity(0.6)],
                                startPoint: .top,
                                endPoint: .bottom
                            )
                        )
                ...
                .chartYAxis {
                    AxisMarks(values: .automatic(desiredCount: 4)) { value in
                        AxisGridLine(stroke: StrokeStyle(lineWidth: 0.5))
                            .foregroundStyle(KynexaTheme.border)
                        AxisValueLabel()
                            .foregroundStyle(KynexaTheme.tertiaryText)
                    }
                }
                ...
                Button {
                    ...
                    .frame(maxWidth: .infinity)
                    .frame(height: 38)
                    .background(KynexaTheme.accent.opacity(0.08))
