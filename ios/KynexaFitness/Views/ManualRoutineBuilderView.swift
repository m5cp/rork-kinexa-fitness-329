...
struct ManualRoutineBuilderView: View {
    ...
    private func dayCard(_ day: ManualRoutineDay, index: Int) -> some View {
                ...
                if day.exercises.isEmpty {
                    ...
                    VStack(spacing: 0) {
                        ForEach(Array(day.exercises.enumerated()), id: \.element.id) { exIdx, exercise in
                            if exIdx > 0 {
                                Divider().overlay(KynexaTheme.border.opacity(0.5)).padding(.leading, 14)
                            }
        ...
        .background(isSelected ? KynexaTheme.accent.opacity(0.04) : KynexaTheme.card)
        .clipShape(.rect(cornerRadius: 16))
        .overlay {
            RoundedRectangle(cornerRadius: 16).stroke(
                isSelected ? KynexaTheme.accent.opacity(0.3) : KynexaTheme.border
            )
        }
    }
    ...
    private var addDayButton: some View {
        Button {
            ...
            .clipShape(.rect(cornerRadius: 14))
            .overlay {
                RoundedRectangle(cornerRadius: 14).stroke(KynexaTheme.accent.opacity(0.2))
            }
        }
        .buttonStyle(PressScaleButtonStyle())
    }
