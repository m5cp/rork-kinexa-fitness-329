        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 12) {
                Image(systemName: workout.isRestDay ? "bed.double.fill" : workoutIcon(for: workout))
                    .font(.body.weight(.semibold))
                    .foregroundStyle(workout.isCompleted ? KynexaTheme.success : KynexaTheme.accent)
                    .frame(width: 40, height: 40)
                    .background((workout.isCompleted ? KynexaTheme.success : KynexaTheme.accent).opacity(0.12))
            ...
            if !workout.isRestDay && !workout.exercises.isEmpty {
                VStack(spacing: 0) {
                    ForEach(workout.exercises.prefix(5)) { exercise in
                        HStack(spacing: 10) {
                            Circle()
                                .fill(exercise.isCompleted ? KynexaTheme.success.opacity(0.6) : KynexaTheme.accent.opacity(0.3))
    ...
    private var emptyState: some View {
        VStack(spacing: 16) {
            Image(systemName: "calendar.badge.exclamationmark")
                .font(.system(size: 40))
                .foregroundStyle(KynexaTheme.accent.opacity(0.4))
