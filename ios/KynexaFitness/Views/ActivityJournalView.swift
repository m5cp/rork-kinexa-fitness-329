...
struct ActivityJournalView: View {
        ...
        return Button {
            ...
            VStack(spacing: 3) {
                Text("\(calendar.component(.day, from: date))")
                    ...
                    .foregroundStyle(
                        isSelected ? .white :
                        !isCurrentMonth ? KynexaTheme.tertiaryText.opacity(0.3) :
                ...
                HStack(spacing: 3) {
                    if hasActivity {
                        Circle()
                            .fill(isSelected ? .white : KynexaTheme.success)
    ...
    private var selectedDayCard: some View {
        ...
        return VStack(alignment: .leading, spacing: 14) {
            ...
            HStack(spacing: 0) {
                miniStat(icon: "figure.mixed.cardio", value: "\(workouts.count + cardio.count)", label: "Workouts", color: KynexaTheme.success)
                miniDivider
                ...
                miniDivider
                miniStat(icon: "shoeprints.fill", value: "\(steps)", label: "Steps", color: KynexaTheme.accent)
            }
            .padding(.vertical, 10)
            .background(KynexaTheme.cardSoft)
            ...
            if workouts.isEmpty && meals.isEmpty && cardio.isEmpty {
                HStack(spacing: 10) {
                    Image(systemName: "calendar.badge.exclamationmark")
                        .font(.title3)
                        .foregroundStyle(KynexaTheme.tertiaryText.opacity(0.5))
    ...
    private func recentDayRow(_ date: Date) -> some View {
        ...
        return HStack(spacing: 14) {
            VStack(spacing: 2) {
                Text(dayAbbrev(date))
                    .font(.system(size: 9, weight: .bold))
                    .foregroundStyle(calendar.isDateInToday(date) ? KynexaTheme.accent : KynexaTheme.tertiaryText)
                Text("\(calendar.component(.day, from: date))")
                    .font(.headline.weight(.bold))
                    .foregroundStyle(calendar.isDateInToday(date) ? KynexaTheme.primaryText : KynexaTheme.secondaryText)
            }
            .frame(width: 36)
        ...
        .padding(14)
        .background(calendar.isDateInToday(date) ? KynexaTheme.accent.opacity(0.06) : KynexaTheme.card)
        .clipShape(.rect(cornerRadius: 16))
        .overlay {
            RoundedRectangle(cornerRadius: 16)
                .stroke(calendar.isDateInToday(date) ? KynexaTheme.accent.opacity(0.2) : KynexaTheme.border)
        }
    }
